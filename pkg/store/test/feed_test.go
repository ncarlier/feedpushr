...
func TestFeedCRUD(t *testing.T) {
	...
	assert.Nil(t, err, "should be nil")
	assert.NotNil(t, page, "should not be nil")
	assert.Equal(t, 1, page.Page, "")
	assert.Equal(t, 10, page.Size, "")
	assert.Equal(t, 1, len(page.Feeds), "unexpected number of feeds")
	assert.Equal(t, "test", page.Feeds[0].ID, "unexpected feed ID")
	assert.Equal(t, 1, page.Total, "unexpected number of feeds")
